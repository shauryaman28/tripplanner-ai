...
async def test_call_tool_success_parsing():
    ...
    mock_response = CallToolResult(
        isError=False, content=[TextContent(type="text", text='{"flights": 10000.0, "total": 20000.0}')]
    )
    ...
    with patch("src.ai.mcp_client.client.get_session", return_value=mock_session):
        ...
        mock_session.call_tool.assert_called_once_with("dummy_tool", arguments={"input": {"param1": "val1"}})
...
async def test_call_tool_returns_tool_error_model():
    ...
    mock_session = AsyncMock()
    mock_response = CallToolResult(
        isError=False, content=[TextContent(type="text", text='{"error": "Budget too low", "code": "BUDGET_TOO_LOW"}')]
    )
    mock_session.call_tool.return_value = mock_response
...
async def test_call_tool_mcp_protocol_error():
    ...
    mock_session = AsyncMock()
    mock_response = CallToolResult(isError=True, content=[TextContent(type="text", text="Validation failed")])
    mock_session.call_tool.return_value = mock_response
